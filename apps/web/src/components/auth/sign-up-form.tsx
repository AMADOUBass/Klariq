    ...
    <div className="w-full max-w-[400px] animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="space-y-1.5">
          <label className="text-[12px] font-medium text-ink-3 ml-1">{t('nameLabel')}</label>
          <div className="relative">
            <User className={cn("absolute left-3 top-1/2 -translate-y-1/2 transition-colors", errors.name ? "text-neg" : "text-ink-4")} size={18} />
            <input
              {...register('name')}
              type="text"
              className={cn(
              ...
              placeholder="Amélie Bernard"
            />
          </div>
          {errors.name && <p className="text-[11px] text-neg ml-1 mt-1">{errors.name.message}</p>}
        </div>
        ...
        <div className="space-y-1.5">
          <label className="text-[12px] font-medium text-ink-3 ml-1">{t('passwordLabel')}</label>
          <div className="relative">
